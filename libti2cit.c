...
uint8_t libti2cit_m_sync_send(uint32_t base, uint8_t addr, uint32_t len, const uint8_t * buf) {
	...
	do {
		if (len) {
			...
			if (buf) HWREG(base + I2C_O_MDR) = *(buf++); // a.k.a. I2CMasterDataPut()
		...
		HWREG(base + I2C_O_MSA) = addr;	// a.k.a. I2CMasterSlaveAddrSet()
		I2CMasterControl(base, cmd);
		while (!I2CMasterBusy(base));	// see http://e2e.ti.com/support/microcontrollers/tiva_arm/f/908/t/368493.aspx
		...
		while (len) {
			HWREG(base + I2C_O_MDR) = *(buf++); // a.k.a. I2CMasterDataPut()
...
uint8_t libti2cit_m_sync_recv(uint32_t base, uint32_t len, uint8_t * buf) {
	...
	if (!len) {	// len cannot be zero. first byte was already received so len is at least 1!
		I2CMasterControl(base, I2C_MASTER_CMD_FIFO_BURST_RECEIVE_ERROR_STOP);
		return 1;
	}

	// first byte already received by i2c hardware: read it, then check len
	while (*(buf++) = HWREG(base + I2C_O_MDR) /* a.k.a. I2CMasterDataGet() */, len) {
		len--;
...
uint8_t libti2cit_m_sync_recvpart(uint32_t base, uint32_t len, uint8_t * buf)
{
	...
	if (mimr & I2C_MIMR_STARTIM) {	// if this is the first time calling libti2cit_m_sync_recvpart()
		...
		*(buf++) = HWREG(base + I2C_O_MDR); /* a.k.a. I2CMasterDataGet() */
		len--;
	} else if (!len) {
